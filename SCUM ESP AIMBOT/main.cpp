...
void GetAddresses()
{
	...
	auto AddrUWorld = Util::FindPattern((PBYTE)Global::BaseAddress, Global::info.SizeOfImage, (PBYTE)"\x48\x8B\x1D\x00\x00\x00\x00\x48\x85\xDB\x74\x3B\x41\xB0\x01", "xxx????xxxxxxxx", 0);
	auto OffUWorld = *reinterpret_cast<uint32_t*>(AddrUWorld + 3);
	Global::m_UWorld = reinterpret_cast<SDK::UWorld * *>(AddrUWorld);
	...
	auto AddrGName = Util::FindPattern((PBYTE)Global::BaseAddress, Global::info.SizeOfImage, (PBYTE)"\x48\x8B\x05\x00\x00\x00\x00\x48\x85\xC0\x75\x5F", "xxx????xxxxx", 0);
	...
	///SDK::FName::GNames = *reinterpret_cast<SDK::TNameEntryArray * *>(AddrGName);
...
DWORD WINAPI UpdateThread(LPVOID)
{
	...
	while (true)
	{
		...
		Global::m_Actors = &Global::m_PersistentLevel->actors;
...
void Aimbot()
{
	...
	if (GetAsyncKeyState(VK_RBUTTON) & 0x8000)
	{
		...
		{
			targetPlayer = Util::GetClosestVisiblePlayer(1000, true);
	...
	if (targetPlayer != nullptr)
	{
		auto pawn = static_cast<SDK::APrisoner*>(targetPlayer);

		SDK::FVector playerLoc;
		Util::Engine::GetBoneLocation(static_cast<SDK::ACharacter*>(targetPlayer)->Mesh, &playerLoc, eBone::Root);

		auto dist = Util::GetDistance(playerController->AcknowledgedPawn->RootComponent->RelativeLocation, playerLoc);
...
void DrawEsp()
{
	...
	{
		SDK::TArray<SDK::AActor*> actors = Global::m_PersistentLevel->actors;
		for (int i = 0; i < actors.Num(); i++)
		{
			SDK::AActor* actor = Global::m_PersistentLevel->actors[i];
			if (actor == nullptr || actor->RootComponent == nullptr)
				continue;

			if (actor->IsA(SDK::APrisoner::StaticClass()))
			{
				SDK::APrisoner* pawn = static_cast<SDK::APrisoner*>(actor);
				...
				Util::Engine::GetBoneLocation(pawn->Mesh, &LeftFootPos, IK_foot_l);
				Util::Engine::GetBoneLocation(pawn->Mesh, &RightFootPos, IK_foot_r);
			    ...
			    //Global::m_LocalPlayer->PlayerController->ProjectWorldLocationToScreen(HeadPos, 0, &ScreenHead);
			    ///Global::m_LocalPlayer->PlayerController->ProjectWorldLocationToScreen(BottomPos, 0, &ScreenBottom);
			...
			else if (actor->IsA(SDK::AZombie2::StaticClass()))
			{
				SDK::AZombie2* pawn = static_cast<SDK::AZombie2*>(actor);
				if (pawn == nullptr)
					continue;
				...
				Global::m_LocalPlayer->PlayerController->ProjectWorldLocationToScreen(pos, 1, &screenPos);
				...
				renderer->drawText(Vec2(screenPos.X - size.x / 2, screenPos.Y), L"Zombie" + distanceText + L"]", YELLOW_COLOR, 0, FONT_SIZE, FONT_TYPE);
			}
		}
	}
}
